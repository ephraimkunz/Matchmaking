...
Subtracting each person's own average removes this structural, answer-driven bias from the ranking, without touching the display score participants actually see. It only removes a person's own outgoing average, though — not how much they're wanted by others in return — so it blunts `person_effect_share` rather than eliminating it; `person_effect_share_calibrated` and `headroom_ranking` in the README's `-d` output show exactly how much survives and what it cost, respectively. Someone with fewer than 2 candidates has no meaningful average to subtract, so calibration is a no-op for them.
...
| Per-person mean-centering (rank score, kept separate from the display score) | Corrects for each person's own answers and weights structurally scoring them high or low against nearly everyone — not a literal rating, since nobody rates anyone directly — so someone with narrow preferences or off-center answers doesn't get an artificial penalty (or edge) purely from that, relative to someone closer to the pool's center. Only removes the outgoing half of the effect, not how much they're wanted in return, so `person_effect_share_calibrated` stays above zero even though it comes in well below the raw `person_effect_share`. Measured cost in absolute compatibility (`headroom_ranking`) has been negligible (99.4%–99.7%) on real runs — subtracting a constant can't reorder a person's own candidate list, it only changes which side binds the `min()` in `combine`, so nobody's actual stated preferences get overridden. |
